...
(function() {
  function initHub() {
    ...
    const showToast = (message, type = 'info') => {
      ...
      toastTimer = setTimeout(() => { state.toast.show = false; }, 4000);
    };
      ...
      return new Promise((resolve) => {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (err) => {
            ...
            // Fallback gracefully to office coordinates
            resolve({ lat: HQ_LAT, lng: HQ_LNG, dist: 10 });
          },
    ...
    // API Layer with Session Recovery
    const API = {
      async req(url, options = {}) {
        const headers = { 'Content-Type': 'application/json' };
        const csrf = window.csrf_token || (typeof frappe !== 'undefined' && frappe.csrf_token);
        ...
        if (data.exc || data.exception) {
          ...
          if (data._server_messages) {
            try {
              const rawMsg = JSON.parse(data._server_messages)[0];
              const parsed = typeof rawMsg === 'string' ? JSON.parse(rawMsg) : rawMsg;
              msg = parsed.message || msg;
              // Clean any HTML from message
              msg = msg.replace(/<[^>]*>?/gm, '');
            } catch(e) {}
          } else if (data.message && typeof data.message === 'string') {
            msg = data.message.replace(/<[^>]*>?/gm, '');
          }
          throw new Error(msg);
        }
        return data;
      ...
      async checkSession() {
        try {
          ...
          if (r.message && r.message !== 'Guest') {
            ...
            await this.loadEmployeeProfile(r.message);
            return true;
          }
        } catch(e) {}
      ...
      async loadEmployeeProfile(userEmail) {
        try {
          const email = userEmail || state.user;
          if (!email) return;
          const r = await this.req(`/api/resource/Employee?filters=[["user_id","=","${email}"]]&fields=["name","employee_name","designation","department","company","leave_approver"]`);
          if (r.data && r.data.length > 0) {
            ...
            state.employeeInfo = {
              ...
              department: emp.department || 'Admin - C&E',
              company: emp.company || 'C and E Consultancy Private Limited',
              leave_approver: emp.leave_approver || 'ujjval.chauhan@candeconsultancy.in'
            };
          }
        } catch(e) {
          console.warn('Profile load fallback:', e.message);
        }
      },
      ...
      async submitLeaveRequest(args) {
        const doc = {
          ...
          leave_approver: 'ujjval.chauhan@candeconsultancy.in',
    ...
    // Component Definition with Pure Inline SVGs
    const App = {
      template: `
        <div id="app" class="font-sans antialiased min-h-screen bg-gray-50 text-gray-900 selection:bg-blue-500 selection:text-white">
          <!-- Global Toast -->
          <transition name="fade">
            <div 
              v-if="state.toast.show" 
              class="fixed top-4 left-4 right-4 z-50 p-3.5 rounded-xl text-white text-xs font-semibold shadow-2xl flex items-center justify-between transition-all"
              :class="{
                'bg-emerald-600': state.toast.type === 'success',
                'bg-rose-600': state.toast.type === 'error',
                'bg-blue-600': state.toast.type === 'info'
              }">
              <span class="flex-1">{{ state.toast.message }}</span>
              <button @click="state.toast.show = false" class="text-white/80 hover:text-white ml-2 font-bold text-sm">✕</button>
            </div>
          </transition>

          <!-- SCREEN 1: LOGIN (When Logged Out) -->
          <div v-if="!state.isLoggedIn" class="min-h-screen flex flex-col justify-center items-center px-4 bg-gradient-to-b from-blue-50 to-gray-100">
            <div class="w-full max-w-sm bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
              <div class="flex flex-col items-center mb-6">
                ...
                <p class="text-xs text-gray-500 mt-1">Self-Service Portal</p>
              </div>
              ...
              <form @submit.prevent="handleLogin" class="space-y-4">
                ...
                <button type="submit" :disabled="loginLoading" class="w-full py-3.5 bg-blue-600 text-white font-semibold rounded-xl shadow-md hover:bg-blue-700 disabled:opacity-50 transition flex justify-center items-center">
          ...
          <div v-else class="min-h-screen pb-20">
            ...
            <div v-if="state.currentView === 'home'">
              ...
              <div class="bg-blue-600 text-white px-5 pt-8 pb-12 rounded-b-3xl shadow-md">
                <div class="flex justify-between items-center mb-3">
                  ...
                  <div class="w-11 h-11 bg-blue-500 rounded-full flex items-center justify-center font-bold text-base border-2 border-blue-400 shadow-sm">
                    {{ (state.employeeInfo.employee_name || 'E').charAt(0) }}
                  </div>
                </div>
              </div>
              ...
              <div class="px-4 -mt-8 space-y-4">
                ...
                <div class="bg-white rounded-2xl p-5 shadow-lg border border-gray-100">
                  <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-xs text-gray-500 font-medium">{{ todayFormatted }}</span>
                  </div>
                  ...
                  <div v-if="!state.todayStatus.isCurrentlyIn" class="space-y-2">
                    <button 
                      ...
                      class="w-full py-4 bg-emerald-600 text-white font-bold text-base rounded-xl shadow-md hover:bg-emerald-700 disabled:opacity-50 transition flex items-center justify-center space-x-2">
                  ...
                  <div v-else class="space-y-2">
                    ...
                    <button 
                      ...
                      :disabled="actionLoading"
                      class="w-full py-3.5 bg-rose-600 text-white font-bold text-sm rounded-xl shadow-md hover:bg-rose-700 disabled:opacity-50 transition flex items-center justify-center space-x-2">
                ...
                <div>
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    <!-- 1. Activity Log -->
                    <div @click="state.currentView = 'activity'" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between cursor-pointer hover:shadow-md transition active:scale-95">
                    ...
                    <!-- 2. Apply Leave -->
                    <div @click="state.currentView = 'leave'" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between cursor-pointer hover:shadow-md transition active:scale-95">
                      <div class="w-10 h-10 bg-blue-100 text-blue-700 rounded-xl flex items-center justify-center mb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/></svg>
                      </div>
                      <div>
                    ...
                    <!-- 3. Work From Home / Overtime -->
                    <div @click="state.attendanceReqType = 'Work From Home'; state.currentView = 'attendance-req'" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between cursor-pointer hover:shadow-md transition active:scale-95">
                    ...
                    <!-- 4. Claim Expense -->
                    <div @click="state.currentView = 'expense'" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between cursor-pointer hover:shadow-md transition active:scale-95">
                    ...
                    <!-- 5. Salary Slips -->
                    <div @click="openSalaryList" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between cursor-pointer hover:shadow-md transition active:scale-95 col-span-2">
                      <div class="flex items-center justify-between">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <!-- VIEW B: CHECK OUT VIEW -->
            <div v-else-if="state.currentView === 'checkout'" class="p-4">
              <div class="flex items-center mb-4">
                <button @click="state.currentView = 'home'" class="p-2 -ml-2 mr-2 text-gray-600 hover:text-gray-900">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
                </button>
            ...
            <div v-else-if="state.currentView === 'expense'" class="p-4">
              ...
              <div class="bg-white rounded-2xl p-5 shadow-md border border-gray-100 space-y-4">
                ...
                <button 
                  ...
                  <span v-if="actionLoading" class="animate-spin mr-2 h-5 w-5 border-2 border-white border-t-transparent rounded-full"></span>
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>
            ...
            <div v-else-if="state.currentView === 'salary'" class="p-4">
              ...
              <div v-else-if="salarySlips.length" class="space-y-3">
                <div 
                  ...
                  class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between cursor-pointer hover:shadow-md transition active:scale-95">
                  <div>
            ...
            <!-- VIEW H: SALARY DETAIL -->
            <div v-else-if="state.currentView === 'salary-detail'" class="p-4">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center">
                  <button @click="state.currentView = 'salary'" class="p-2 -ml-2 mr-2 text-gray-600 hover:text-gray-900">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
                  </button>
                  <h2 class="text-lg font-bold text-gray-900">Salary Slip</h2>
                </div>
                <button @click="printSlip" class="text-xs bg-blue-600 text-white font-semibold px-3 py-1.5 rounded-lg flex items-center space-x-1 hover:bg-blue-700 transition">
            ...
            <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around items-center h-16 z-50 shadow-lg px-4">
              <button @click="openHistory" :class="state.currentView === 'history' ? 'text-blue-600 font-semibold' : 'text-gray-500'" class="flex flex-col items-center flex-1 py-1 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg>
      ...
      setup() {
        ...
        const handleLogin = async () => {
          ...
          try {
            ...
            state.user = loginEmail.value.trim();
            localStorage.setItem('ce_hub_auth', '1');
            showToast('Welcome to C&E Hub!', 'success');
            await API.loadEmployeeProfile(loginEmail.value.trim());
        ...
        const viewSalaryDetail = async (slipName) => {
          ...
          try {
            ...
            showToast('Failed to load salary slip detail', 'error');
          } finally {
            salaryLoading.value = false;
          }
        };
        ...
        const openHistory = async () => {
          ...
          try {
            ...
            activityHistory.value = acts;
            requestHistory.value = reqs;
          } catch(e) {
